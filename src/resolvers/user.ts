...
async function getLocationFromIp(ip: string): Promise<LocationData> {
  ...
  try {
    ...
    const response = await fetch(
      `https://ip-api.com/json/${ip}?fields=status,message,country,regionName,city,lat,lon,query`
    );
...
export class UserResolver {
  ...
  async verifyCode(
    ...
    if (!storedEmailCode || input.code !== storedEmailCode) {
      return {
        errors: [
          { field: "code", message: "Invalid or expired verification code" },
        ],
      };
    }
  ...
  async login(
    ...
    if (!user.isEmailVerified) {
      return {
        errors: [{ field: "verification", message: "User not verified" }],
      };
    }
