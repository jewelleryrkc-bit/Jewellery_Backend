...
export class OrderResolver {
  ...
  @Query(() => Order, { nullable: true })
   ...
   // In your OrderResolver's createOrder method
    @Mutation(() => Order)
      ...
      // In your OrderResolver, update the createOrder mutation:
  // @Mutation(() => Order)
  //   async createOrder(@Ctx() { em, req }: MyContext): Promise<Order> {
  //     if (!req.session.userId) throw new Error("Not authenticated");
  ...
  //     // 🎯 GET ALL ACTIVE DISCOUNTS FOR THIS STORE
  //     const discounts = await em.find(
  //       Discount,
  //       { 
  //         company: { id: req.session.companyId }, // Assuming companyId in session
  //         isActive: true,
  //         status: "active"
  //       },
